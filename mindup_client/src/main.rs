...
use mindup_common::{Response as MindupResponse, Task};
...
use yew::{classes, events::InputEvent, html, Callback, Component, Context, Html, KeyboardEvent, NodeRef};
...
impl Component for Root {
    ...
    fn update(&mut self, ctx: &Context<Self>, msg: Self::Message) -> bool {
        match msg {
            ...
            Msg::TypeNew => {
                let value = dom::existing::get_element_by_id::<HtmlInputElement>("new-task-input").value();
            ...
            Msg::TypeEdit(idx) => {
                if let Some(edit) = &mut self.state.edit {
                    let value =
                        dom::existing::get_element_by_id::<HtmlInputElement>(&format!("edit-task-{}", idx)).value();
                    edit.value = value;
                }
                false
            },
            ...
            Msg::Remove(idx) => {
                ...
                Self::send_post(ctx, format!("/mindup/delete/{}", idx + 1), "");
                false
            },
            ...
            Msg::ClearCompleted => {
                Self::send_post(ctx, "/mindup/clear_completed", "");
                false
            },
            Msg::Focus => {
            ...
            Msg::Fetch(MindupResponse::List(list)) => {
                self.state.list = list;
                true
            },
            Msg::Fetch(MindupResponse::Task(task)) => {
                self.state.list.push(task);
                true
            },
            Msg::Fetch(MindupResponse::Empty) => true,
            Msg::Fetch(MindupResponse::Error(err)) => {
...
fn callback(ctx: &Context<Root>) -> Callback<Result<(Response, Result<MindupResponse>)>> {
    ctx.link()
        .callback(|response_result: Result<(Response, Result<MindupResponse>)>| {
            response_result
                .map(|(response, body)| {
